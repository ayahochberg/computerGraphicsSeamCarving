...
public class ImageProcessor extends FunctioalForEachLoops {
	...
	public BufferedImage greyscale() {
		BufferedImage outputImage = newEmptyInputSizedImage();

		this.setForEachInputParameters();
		forEach((y, x) -> {
			Color currentColor = new Color(workingImage.getRGB(x, y));
			int red = rgbWeights.redWeight * currentColor.getRed();
			int green = rgbWeights.greenWeight * currentColor.getGreen();
			int blue = rgbWeights.blueWeight * currentColor.getBlue();
			int greyScale = (red + green + blue) / (rgbWeights.redWeight + rgbWeights.greenWeight + rgbWeights.blueWeight);

			outputImage.setRGB(x, y, new Color(greyScale, greyScale, greyScale).getRGB());
		});
	...
	public BufferedImage nearestNeighbor() {
		...
		double widthRatio = Math.round((double) inWidth / (double) outWidth);
		double heightRatio =  Math.round((double) inHeight / (double) outHeight);

		this.setForEachParameters(outWidth, outHeight);

		forEach((y,x) ->{
			int x_neighbor = (int) (x * widthRatio);
			int y_neighbor = (int) (y * heightRatio);
			Color color = new Color(workingImage.getRGB(x_neighbor, y_neighbor));

...
public class MenuWindow extends JFrame implements Logger {
	...
	private int getMaxTrueValue(boolean[][] srcMask){
		int maxTrueValues = 0;

		for (boolean[] booleans : srcMask) {
			int sum = 0;
			for (boolean b : booleans) {
				sum += b ? 1 : 0;
			}

			maxTrueValues = Math.max(sum, maxTrueValues);
		}

		return maxTrueValues;
	}

	public void removeObjectFromImage(boolean[][] srcMask) {
		int inWidth = workingImage.getWidth();
		int outWidth = scaleSelector.width();
		RGBWeights rgbWeights = colorMixer.getRGBWeights();
		int numOfSeams = Math.min((inWidth / 3) - 1, this.getMaxTrueValue(srcMask));
		SeamsCarver sc_1 = new SeamsCarver(this, duplicateImage(), inWidth - numOfSeams, rgbWeights, srcMask);
		BufferedImage img;

		img = sc_1.resize();
		SeamsCarver sc_2 = new SeamsCarver(this, img, outWidth, rgbWeights, sc_1.getMaskAfterSeamCarving());
		img = sc_2.resize();
